              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6">
                <div className="bg-gray-50 rounded-lg p-3 sm:p-4">
                  ...
                  <p className="text-base sm:text-lg font-bold text-gray-900 mt-1 break-words">
                    ...
                    {num(statement.opening_balance).toFixed(3)}
                  </p>
                </div>
                <div className="bg-gray-50 rounded-lg p-3 sm:p-4">
                  <p className="text-xs text-gray-500">Total Deposits</p>
                  <p className="text-base sm:text-lg font-bold text-green-600 mt-1 break-words">
                    {statement.account.currency}{" "}
                    {num(statement.total_deposits).toFixed(3)}
                  </p>
                </div>
                <div className="bg-gray-50 rounded-lg p-3 sm:p-4">
                  <p className="text-xs text-gray-500">Total Withdrawals</p>
                  <p className="text-base sm:text-lg font-bold text-red-600 mt-1 break-words">
                    {statement.account.currency}{" "}
                    {num(statement.total_withdrawals).toFixed(3)}
                  </p>
                </div>
                <div className="bg-gray-50 rounded-lg p-3 sm:p-4">
                  <p className="text-xs text-gray-500">Closing Balance</p>
                  <p className="text-base sm:text-lg font-bold text-blue-600 mt-1 break-words">
                    {statement.account.currency}{" "}
                    {num(statement.closing_balance).toFixed(3)}
                  </p>
                </div>
              </div>
                      ...
                      {statement.transactions.map(
                        (trans: any, index: number) => (
                          <tr key={index} className="hover:bg-gray-50">
                            ...
                            <td className="px-4 py-2 text-right text-sm font-mono text-red-600 whitespace-nowrap">
                              {trans.debit > 0
                                ? `${statement.account.currency} ${num(trans.debit).toFixed(3)}`
                                : "-"}
                            </td>
                            <td className="px-4 py-2 text-right text-sm font-mono text-green-600 whitespace-nowrap">
                              {trans.credit > 0
                                ? `${statement.account.currency} ${num(trans.credit).toFixed(3)}`
                                : "-"}
                            </td>
                            <td className="px-4 py-2 text-right text-sm font-mono font-bold text-blue-600 whitespace-nowrap">
                              {statement.account.currency}{" "}
                              {num(trans.balance).toFixed(3)}
                            </td>
                          </tr>
                        ),
                      )}
