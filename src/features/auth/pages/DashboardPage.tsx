...
import { useGetPendingApprovalsQuery as useGetPurchasePendingApprovalsQuery } from "../../../services/purchaseApi";
...
export default function DashboardPage() {
  ...
  const { data: activityLogsData, isLoading: logsLoading } =
    ...
    const totalRevenue =
      ...
      breakdownData.reduce(
        (sum: number, channel: any) =>
          sum + (parseFloat(channel.total_revenue) || 0),
        0,
      );
  ...
  const revenueChange = useMemo(() => {
    if (selectedTimeframe === "weekly")
      return (
        parseFloat(weeklyData?.data?.comparison?.revenue_change_percentage) || 0
      );
    return (
      parseFloat(monthlyData?.data?.comparison?.revenue_change_percentage) || 0
    );
  }, [weeklyData, monthlyData, selectedTimeframe]);
  ...
  const pendingOrdersCount = useMemo(
    () =>
      weeklyData?.data?.summary?.pending_orders ||
      monthlyData?.data?.summary?.pending_orders ||
      0,
    ...
    },
    {
      ...
      sub:
        pendingApprovalsCount > 0
          ? `${pendingApprovalsCount} requests awaiting approval`
          ...
          {statCards.map((card) => (
            ...
            >
              ...
              <div className="flex items-center mt-2 gap-1">
                ...
                />
                <p
                  className={`text-xs md:text-sm font-semibold truncate ${card.subColor}`}
                >
                  {card.sub}
                </p>
              </div>
                ...
                <div className="relative w-48 h-48 md:w-56 md:h-56 xl:w-64 xl:h-64 mx-auto mb-4 md:mb-6">
                  {channelPieData.length > 0 ? (
                    channelPieData
                      ...
                      .map((channel: any, index: number) => {
                        const totalValue = channelPieData.reduce(
                          (sum: number, item: any) => sum + item.value,
                          0,
                        );
                        const percentage =
                          totalValue > 0
                            ? (channel.value / totalValue) * 100
                            : 0;
                  ...
                  <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center">
                    <div className="text-2xl md:text-3xl font-bold text-gray-900">
                      {channelPieData.length > 0
                        ? Math.round(
                            (channelPieData.reduce(
                              (acc: number, channel: any) =>
                                acc + channel.value,
                              0,
                            ) /
                              (channelPieData.reduce(
                ...
                <div className="space-y-3">
                  {channelPieData.length > 0 ? (
                    channelPieData.map((channel: any, idx: number) => {
                      ...
                      const percentage =
                        totalValue > 0
                          ? ((channel.value / totalValue) * 100).toFixed(1)
                          : "0";
                      const colors = [
                        "#91C0EECC",
                        "#1773CF99",
                    ...
                    activityLogs.map((log: any) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-4 md:px-5 py-3 md:py-4 whitespace-nowrap text-sm text-gray-700">
                            {row.module}
                          </td>
                          ...
                          <td className="px-4 md:px-5 py-3 md:py-4 whitespace-nowrap">
                            <span
                              className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${row.status.color}`}
                            >
                ...
                <div className="flex items-center gap-1">
                  ...
                  {pageNumbers.map((num) => (
                    <button
                      ...
                      className={`w-8 h-8 text-xs md:text-sm font-medium rounded-lg transition-colors ${currentPage === num ? "bg-blue-600 text-white" : "text-gray-700 hover:bg-gray-100"}`}
                    >
                      {num}
                    </button>
                  ))}
                  ...
                  .map((item: any, index: number) => (
                    ...
                    >
                      <div className="flex items-start justify-between gap-2 mb-2">
                        ...
                        <div className="text-right shrink-0">
                          <div
                            className={`text-sm font-bold px-2 py-1 rounded-lg ${item.current_stock === 0 ? "bg-red-100 text-red-600" : "bg-yellow-100 text-yellow-600"}`}
                          >
