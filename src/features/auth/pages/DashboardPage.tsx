...
export default function DashboardPage() {
  ...
  const channelPieData = useMemo(() => {
    ...
    return channelData.data.channel_breakdown.map((channel: any) => ({
      ...
      percentage: (
        (parseFloat(channel.total_revenue) /
          (channelData.data.summary?.total_revenue || 1)) *
        100
      ).toFixed(1),
    }));
  }, [channelData]);
      ...
      <div className="space-y-4 md:space-y-6 overflow-x-hidden min-w-0">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 md:gap-6">
                ...
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <YAxis
                      ...
                      domain={
                        processedSalesData.length === 0 ? [0, 10] : undefined
                      }
                    />
                    <Tooltip
                      content={
                    ...
                    <Bar
                      dataKey="revenue"
                      fill={
                        processedSalesData.length > 0 ? "#00C0E8" : "#E5E7EB"
                      }
                      radius={[8, 8, 8, 8]}
                      maxBarSize={40}
                    />
          ...
          <div className="bg-white rounded-lg p-4 md:p-5 xl:p-6 min-w-0">
            <h3 className="text-sm md:text-base xl:text-lg font-semibold text-gray-900 mb-4">
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      ...
                      {channelPieData.length > 0 && (
                        <Tooltip
                          formatter={(value: any) =>
                            `KWD ${parseFloat(value).toFixed(3)}`
                          }
                        />
                      )}
                      {channelPieData.length > 0 && (
          ...
          <div className="bg-white rounded-xl p-4 md:p-5 xl:p-6 min-w-0">
            ...
            <div className="mt-4 pt-3 border-t border-gray-100">
              <button className="w-full py-2 text-blue-600 hover:text-blue-800 hover:underline font-medium text-xs md:text-sm transition-colors">
                View All Alerts (
                {lowStockLoading ? "..." : statistics.lowStockCount}) →
              </button>
            </div>
          </div>
        </div>
