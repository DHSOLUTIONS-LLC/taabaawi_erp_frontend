      ...
      <div className="bg-white rounded-xl w-full max-w-3xl max-h-[90vh] flex flex-col shadow-xl">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-semibold text-gray-800">Merge Customers</h2>
            <span className="text-xs bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded-full">
              {selectedDuplicate.similarity_score}% match
            </span>
          </div>
          <button onClick={() => dispatch(closeDuplicateModal())} className="p-1 hover:bg-gray-100 rounded">
            <X className="h-4 w-4" />
          </button>
        </div>
              ...
              {COMPARE_FIELDS.map(({ label, key }) => {
                ...
                return (
                  <div key={key} className={`grid grid-cols-3 text-sm ${differs ? 'bg-yellow-50' : ''}`}>
                    ...
                    <div className={`px-3 py-2 border-l border-gray-200 ${primaryId === c1.id && differs ? 'font-medium text-blue-700' : 'text-gray-700'}`}>
                      {v1}
                    </div>
                    <div className={`px-3 py-2 border-l border-gray-200 ${primaryId === c2.id && differs ? 'font-medium text-blue-700' : 'text-gray-700'}`}>
                      {v2}
                    </div>
                  </div>
                );
        ...
        <div className="flex items-center justify-end gap-2 px-5 py-4 border-t border-gray-100">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-5 py-2 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-40"
          >
