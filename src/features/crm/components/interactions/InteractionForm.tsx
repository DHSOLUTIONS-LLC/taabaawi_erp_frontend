...
export const InteractionForm = () => {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm<Partial<CustomerInteraction>>();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-xl">
        <div className="sticky top-0 bg-white flex items-center justify-between px-4 sm:px-5 py-3 sm:py-4 border-b border-gray-100">
          <h2 className="font-semibold text-gray-800 text-sm sm:text-base">Log Interaction — {customer.full_name}</h2>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-4 sm:p-5 space-y-4">
          ...
          <div className="flex flex-col-reverse sm:flex-row justify-end gap-2 pt-2">
            <button
              type="button"
              onClick={() => dispatch(closeInteractionModal())}
