            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {interactions.map((interaction: any) => {
                  const Icon =
                    typeIcon[
                      interaction.interaction_type as keyof typeof typeIcon
                    ] ?? MoreHorizontal;
                  return (
                    <tr
                      key={interaction.id}
                    ...
                    >
                      <td className="px-3 py-2.5">
                        <div
                          className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-full text-xs font-medium ${typeColor[interaction.interaction_type as keyof typeof typeColor] ?? "bg-gray-50 text-gray-500"}`}
                        >
                      ...
                      <td
                        className="px-3 py-2.5 text-gray-500 text-sm max-w-xs truncate"
                ...
                <div key={interaction.id} className="bg-gray-50 rounded-lg p-3">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <span className="text-xs text-gray-400">
                      {new Date(
                        interaction.interaction_date ?? interaction.created_at,
                      ).toLocaleDateString()}
                    </span>
                  </div>
