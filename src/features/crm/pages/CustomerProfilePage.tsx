...
export const CustomerProfilePage = () => {
  ...
  if (!customer) return (
    <div className="p-4 sm:p-6 text-center text-gray-400">Customer not found.</div>
  );
            ...
            <div className="flex gap-1 min-w-max">
              {TABS.map(({ id: tabId, label, icon: Icon }) => (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 sm:px-4 py-2.5 text-xs sm:text-sm font-medium border-b-2 -mb-px transition-colors ${activeTab === tabId
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-3 text-sm">
                  {[
                    ...
                    <React.Fragment key={label}>
                      ...
                      <span className="text-gray-700 font-medium text-xs sm:text-sm break-words">{value}</span>
                    </React.Fragment>
                  ))}
                </div>
              </div>
              ...
              {customer.notes && (
                <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-5">
                  ...
                  <p className="text-xs sm:text-sm text-gray-600 break-words">{customer.notes}</p>
                </div>
              )}
            </div>
          )}
                  ...
                  <table className="w-full text-sm min-w-[600px]">
                    <thead className="bg-gray-50 border-b border-gray-200">
                      <tr>
                        {['Order #', 'Date', 'Channel', 'Total', 'Status'].map(h => (
                          <th key={h} className="px-3 sm:px-4 py-3 text-left font-medium text-gray-500 text-xs sm:text-sm whitespace-nowrap">
                            {h}
                          </th>
                        ))}
                      </tr>
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {ordersLoading
                        ...
                        : orders.length === 0
                          ? <tr><td colSpan={5} className="px-3 sm:px-4 py-8 text-center text-gray-400">No orders found</td></tr>
                          : orders.map((o: any) => (
                            <tr key={o.id} className="hover:bg-gray-50">
                              <td className="px-3 sm:px-4 py-3 font-medium text-blue-600 text-xs sm:text-sm whitespace-nowrap">#{o.order_number ?? o.id}</td>
                              <td className="px-3 sm:px-4 py-3 text-gray-500 text-xs sm:text-sm whitespace-nowrap">{new Date(o.created_at).toLocaleDateString()}</td>
                              <td className="px-3 sm:px-4 py-3 text-gray-500 text-xs sm:text-sm">{o.channel ?? '-'}</td>
                              <td className="px-3 sm:px-4 py-3 text-gray-700 text-xs sm:text-sm whitespace-nowrap">${o.total_amount?.toLocaleString()}</td>
                              <td className="px-3 sm:px-4 py-3">
                                <span className="px-2 py-0.5 rounded-full text-xs bg-gray-100 text-gray-600 whitespace-nowrap">{o.order_status}</span>
                              </td>
                            </tr>
                          ))
                      }
