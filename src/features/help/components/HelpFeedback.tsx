  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto">
      <div className="flex items-center justify-center min-h-screen px-3 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full w-[calc(100%-2rem)] mx-auto">
          <div className="bg-white px-3 sm:px-4 pt-4 pb-3 sm:p-6 sm:pb-4">
            ...
            {submitted ? (
              <div className="text-center py-4 sm:py-6">
                <div className="mb-3 sm:mb-4 text-green-500 text-3xl sm:text-5xl">✓</div>
                <p className="text-sm sm:text-base text-gray-700 mb-1 sm:mb-2">Thank you for your feedback!</p>
                <p className="text-xs sm:text-sm text-gray-500">It helps us improve our content.</p>
              </div>
            ) : (
              <>
                ...
                <div className="flex flex-col xs:flex-row justify-center gap-3 xs:space-x-4 mb-4 sm:mb-6">
                  <button
                    onClick={() => setFeedbackType('helpful')}
                    className={`flex items-center justify-center gap-2 p-3 sm:p-4 rounded-lg border-2 transition-colors ${feedbackType === 'helpful'
                        ? 'border-green-500 bg-green-50'
                        : 'border-gray-200 hover:border-green-200'
                      }`}
                  >
                    <span className="text-2xl sm:text-3xl">👍</span>
                    <span className="text-xs sm:text-sm font-medium">Yes, helpful</span>
                  </button>

                  <button
                    onClick={() => setFeedbackType('not_helpful')}
                    className={`flex items-center justify-center gap-2 p-3 sm:p-4 rounded-lg border-2 transition-colors ${feedbackType === 'not_helpful'
                        ? 'border-red-500 bg-red-50'
                        : 'border-gray-200 hover:border-red-200'
                      }`}
                  >
                    <span className="text-2xl sm:text-3xl">👎</span>
                    <span className="text-xs sm:text-sm font-medium">No, not helpful</span>
                  </button>
                </div>
