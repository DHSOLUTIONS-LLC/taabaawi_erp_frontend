            ...
            <div className="space-y-6 p-4 md:p-6">
                ...
                <div className="flex flex-row justify-between items-center">
                    <Link to={`${basePath}/hr`} className="flex flex-row items-center">
                        ...
                        <span className="px-2 font-semibold text-sm md:text-base">Payroll Management</span>
                    </Link>
                    <div className="flex space-x-3">
                        <Link to={`${basePath}/hr/payrolls/generate`}>
                            <button className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
                                Generate Payroll
                            </button>
                        </Link>
                        <Link to={`${basePath}/hr/payrolls/generate-bulk`}>
                ...
                {statistics && (
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div className="bg-white rounded-xl p-6 shadow-sm">
                            <p className="text-sm text-gray-500 mb-2">Total Payrolls</p>
                            <p className="text-2xl font-bold text-gray-900">{statistics.total_payrolls || 0}</p>
                            <div className="flex mt-2 text-xs">
                                <span className="text-gray-500 mr-3">Draft: {statistics.draft || 0}</span>
                                <span className="text-green-500 mr-3">Approved: {statistics.approved || 0}</span>
                                <span className="text-blue-500">Paid: {statistics.paid || 0}</span>
                            </div>
                        </div>
                        <div className="bg-white rounded-xl p-6 shadow-sm">
                            <p className="text-sm text-gray-500 mb-2">Total Basic Salary</p>
                            <p className="text-2xl font-bold text-gray-900">{formatCurrency(statistics.total_basic_salary || '0')}</p>
                        </div>
                        <div className="bg-white rounded-xl p-6 shadow-sm">
                            <p className="text-sm text-gray-500 mb-2">Total Allowances & Bonuses</p>
                            <p className="text-2xl font-bold text-gray-900">
                                {formatCurrency((parseFloat(statistics.total_allowances || '0') + parseFloat(statistics.total_bonuses || '0')).toString())}
                            </p>
                        </div>
                        <div className="bg-white rounded-xl p-6 shadow-sm">
                            <p className="text-sm text-gray-500 mb-2">Total Net Salary</p>
                            <p className="text-2xl font-bold text-green-600">{formatCurrency(statistics.total_net_salary || '0')}</p>
                        </div>
                    </div>
                )}
                ...
                <div className="bg-white rounded-xl p-6 shadow-sm">
                    <div className="flex flex-wrap md:flex-nowrap items-center gap-4">
                        {/* Month Filter - Now with "All Months" option */}
                        <div className="flex-1 min-w-50 relative">
                            <select
                                value={selectedMonth}
                                onChange={(e) => setSelectedMonth(e.target.value)}
                                className="w-full px-4 py-2.5 border border-gray-300 rounded-lg appearance-none bg-white pr-10 focus:ring-2 focus:ring-blue-500"
                            >
                        ...
                        <div className="shrink-0">
                            <button
                                ...
                                title="Reset Filters"
                            >
                                <img src={filterIcon} alt="Reset" className="w-5 h-5" />
                            </button>
                        </div>
                    </div>
