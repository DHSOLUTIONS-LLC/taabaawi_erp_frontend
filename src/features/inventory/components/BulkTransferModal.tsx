...
export default function BulkTransferModal({ isOpen, onClose, selectedProducts }: BulkTransferModalProps) {
    ...
    const [productsStock, setProductsStock] = useState<Map<number, BranchStock[]>>(new Map());
    ...
    // Mutation
    const [createTransfer, { isLoading: transferring }] = inventoryApi.endpoints.createStockTransfer.useMutation();
    ...
    const handleSubmit = async () => {
        ...
        const invalidQuantities = productQuantities.filter(pq => pq.quantity <= 0 || pq.quantity > pq.available);
        ...
        try {
            const fromBranchName = branches.find((b: any) => b.id === fromBranchId)?.branch_name;
            const toBranchName = branches.find((b: any) => b.id === toBranchId)?.branch_name;

            const fromType = branches.find((b: any) => b.id === fromBranchId)?.branch_type;
            ...
            const result = await createTransfer({
                ...
                notes: notes || `Bulk transfer of ${productQuantities.length} products from ${fromBranchName} to ${toBranchName}`,
                items: productQuantities.map(pq => ({
                    product_id: pq.product_id,
                    variant_id: null,
                    quantity: pq.quantity,
                    notes: selectedProducts.find(p => p.id === pq.product_id)?.name || '',
                })),
            }).unwrap();

            if (result.success) {
                alert(`Successfully transferred ${productQuantities.length} products from ${fromBranchName} to ${toBranchName}`);
                onClose();
            }
        } catch (error: any) {
                    ...
                    <div className="flex items-start space-x-3 bg-blue-50 rounded-lg p-4 border border-blue-200 mb-6">
                        ...
                        <div className="flex-1">
                            ...
                            <p className="text-xs text-blue-700 mt-1">
                                Select source branch to see available stock. Only products with available inventory will be shown.
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="mb-6">
                        ...
                        {fromBranchId && (
                            <p className="text-xs text-blue-600 mt-2">
                                Available in {fromBranchName}: {availableProducts.length} of {selectedProducts.length} products have stock
                            </p>
                        )}
                    </div>
                        ...
                        <div className="mb-6">
                            ...
                            {availableProducts.length === 0 ? (
                                <div className="p-8 text-center bg-gray-50 rounded-lg border border-gray-200">
                                    ...
                                    <h3 className="text-sm font-medium text-gray-900 mb-1">No Available Stock</h3>
                                    <p className="text-xs text-gray-500">
                                        None of the selected products have available inventory in {fromBranchName}.
                                    </p>
                                </div>
                            ) : (
                                    ...
                                    <table className="min-w-full divide-y divide-gray-200">
                                        ...
                                        <tbody className="bg-white divide-y divide-gray-200">
                                            {availableProducts.map((product) => {
                                                const pq = productQuantities.find(q => q.product_id === product.id);
                ...
                <div className="px-8 py-6 border-t border-gray-200 shrink-0 bg-white rounded-b-2xl">
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <button
                            ...
                            disabled={transferring || !fromBranchId || !toBranchId || productQuantities.length === 0}
