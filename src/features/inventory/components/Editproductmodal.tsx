...
import { useCreateProductMutation, useUpdateProductMutation, useGetCategoriesQuery } from '../../../services/inventoryApi';
...
export default function EditProductModal({ isOpen, onClose, mode, product }: EditProductModalProps) {
    ...
    const [lowStockAlert, setLowStockAlert] = useState(product?.low_stock_alert?.toString() || '10');
    ...
    const handleSaveProduct = async () => {
        ...
        try {
            ...
            if (hasVariants && variants.length > 0) {
                variants.forEach((variant, index) => {
                    if (variant.variant_name && variant.variant_value) {
                        ...
                        formData.append(`variants[${index}][additional_price]`, variant.additional_price.toString());
            ...
            console.log(`📤 ${mode === 'edit' ? 'Updating' : 'Creating'} product:`, product?.id);
                    ...
                    <div className="border-t border-gray-200 pt-6">
                        <div className="flex items-center justify-between mb-4">
                            <div>
                                ...
                                <p className="text-xs text-gray-500 mt-1">
                                    Enable if this product has multiple variations (size, color, storage, etc.)
                                </p>
                            </div>
                            <button
                                type="button"
                        ...
                        {hasVariants && (
                            <div className="space-y-4 border border-gray-200 rounded-lg p-4 bg-gray-50">
                                ...
                                {variants.map((variant, index) => (
                                    <div key={index} className="grid grid-cols-1 md:grid-cols-5 gap-4 p-4 border border-gray-200 rounded-lg bg-white">
                                        <div>
                                ...
                                <p className="text-xs text-gray-500 mt-2">
                                    Note: SKU and barcode for variants will be auto-generated by the system
                                </p>
                            </div>
                        )}
                    </div>
