...
export default function TransferStockModal({ isOpen, onClose, product }: TransferStockModalProps) {
    ...
    const finalQuantity = isCustomQuantity ? parseInt(customQuantity) || 0 : quantity;
        ...
        <>
            ...
            <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[90%] max-w-200 bg-white rounded-2xl shadow-2xl z-70 flex flex-col h-[90vh]">
                ...
                <div className="px-8 pt-6 pb-4 border-b border-gray-200 shrink-0">
                    <h2 className="text-xl font-bold text-gray-900 text-center">TRANSFER STOCK</h2>
                    ...
                    <div className="px-8 py-6 space-y-6">
                        ...
                        <div className="flex items-start space-x-3 bg-blue-50 rounded-lg p-4 border border-blue-200">
                            ...
                            <div className="flex-1">
                                <p className="text-sm font-medium text-blue-900">
                                    Total Stock: {totalQuantity} units | Available: {totalAvailable} units
                                </p>
                                {fromBranchId && (
                                    <p className="text-xs text-blue-700 mt-1">
                                        Available in {fromBranchStock?.branch_name}: {maxAvailable} units
                                    </p>
                                )}
                                {fromBranchId && toBranchId && (
                                    <p className="text-xs text-blue-700 mt-1">
                                        Transfer Type: <span className="font-semibold">{getTransferType()}</span>
                                    </p>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="bg-white shadow-md rounded-lg border border-gray-200">
                            <div className="px-4 py-3 bg-gray-50 border-b border-gray-200">
                                <h3 className="text-base font-bold text-gray-900">Stock By Branch</h3>
                            </div>
                            ...
                            {!stockLoading && !stockError && branchStock.length === 0 && (
                                <div className="p-12 text-center">
                                    ...
                                    <p className="text-gray-500">This product has no inventory in any branch.</p>
                                </div>
                            )}
                                                    ...
                                                    <td className="px-6 py-4 whitespace-nowrap">
                                                        <div className="text-sm font-medium text-gray-900">
                                                            ...
                                                            {branch.branch_id === fromBranchId && (
                                                                <span className="ml-2 text-xs text-blue-600 font-semibold">FROM</span>
                                                            )}
                                                            {branch.branch_id === toBranchId && (
                                                                <span className="ml-2 text-xs text-green-600 font-semibold">TO</span>
                                                            )}
                                                        </div>
                                                    </td>
                                                    <td className="px-6 py-4 whitespace-nowrap">
                                                ...
                                                {branchStock
                                                    ...
                                                    .map((branch) => (
                                                        <option key={branch.branch_id} value={branch.branch_id}>
                                                            {branch.branch_name} ({branch.available} available)
                                                        </option>
                                                    ))}
                                            </select>
