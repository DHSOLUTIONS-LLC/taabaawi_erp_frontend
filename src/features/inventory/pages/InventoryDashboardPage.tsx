...
export default function DashboardPage() {
  ...
  const [activeTab, setActiveTab] = useState<"products" | "inventory">(
    "products",
  );
  ...
  const { data: allInventoryResponse, isLoading: allInventoryLoading } =
    useGetAllInventoryQuery();
  ...
  const sortProducts = (products: Product[]) => {
    return [...products].sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case "cost_price":
          aValue =
            typeof a.cost_price === "string"
              ? parseFloat(a.cost_price)
              : a.cost_price || 0;
          bValue =
            typeof b.cost_price === "string"
              ? parseFloat(b.cost_price)
              : b.cost_price || 0;
          break;
        case "selling_price":
          aValue =
            typeof a.selling_price === "string"
              ? parseFloat(a.selling_price)
              : a.selling_price || 0;
          bValue =
            typeof b.selling_price === "string"
              ? parseFloat(b.selling_price)
              : b.selling_price || 0;
          break;
        case "created_at":
        ...
        case "stock":
          aValue =
            a.inventory?.reduce((sum, inv) => sum + (inv.quantity || 0), 0) ||
            0;
          bValue =
            b.inventory?.reduce((sum, inv) => sum + (inv.quantity || 0), 0) ||
            0;
          break;
        default:
          return 0;
                  ...
                  {productsLoading ? (
                    ...
                    currentProducts.map((product: Product) => (
                      ...
                      >
                        ...
                        <td className="px-4 md:px-5 py-3 md:py-4 whitespace-nowrap">
                          <div className="w-10 h-10 rounded-xl overflow-hidden border border-gray-100">
                            <img
                              ...
                              onError={(e) => {
                                e.currentTarget.src =
                                  "https://images.unsplash.com/photo-1457089328109-e5d9bd499191?w=500&auto=format&fit=crop&q=60";
                              }}
                            />
                          </div>
                        </td>
                        ...
                        {activeTab === "products" ? (
                          <>
                            <td className="px-4 md:px-5 py-3 md:py-4 whitespace-nowrap text-sm text-gray-600">
                              {product.unit || "N/A"}
                            </td>
                            <td className="px-4 md:px-5 py-3 md:py-4 whitespace-nowrap text-sm text-gray-600">
                              {product.weight || "0"}
            ...
            activeTab === "products" && (
              <div className="px-4 sm:px-6 py-5 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="text-sm text-gray-500">
                  ...
                  {Math.min(indexOfLastProduct, productsToDisplay.length)} of{" "}
