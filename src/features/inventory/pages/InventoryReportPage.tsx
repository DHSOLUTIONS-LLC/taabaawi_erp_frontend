...
export default function InventoryReportsPage() {
  ...
  const { data: inventoryResponse, isLoading: inventoryLoading } =
    inventoryApi.endpoints.getInventoryMovements.useQuery(
      {
        page: 1,
      },
      {
        skip:
          activeTab !== "stock-summary" && activeTab !== "inventory-movement",
      },
    );

  // Fetch low stock data
  const { data: lowStockResponse, isLoading: lowStockLoading } =
    inventoryApi.endpoints.getLowStockProducts.useQuery(undefined, {
      skip: activeTab !== "low-stock",
    });

  // Fetch transfer history
  const { data: transferHistoryResponse, isLoading: transferHistoryLoading } =
    inventoryApi.endpoints.getStockTransfers.useQuery(
      {
        page: 1,
      },
      {
  ...
  const { data: damageMovements, isLoading: damageMovementsLoading } =
    inventoryApi.endpoints.getInventoryMovements.useQuery(
      {
        page: 1,
      },
      {
        skip: activeTab !== "damage-discard",
      },
    );
  ...
  const transferHistoryData: TransferHistoryItem[] = useMemo(() => {
    ...
    return transfers.map((transfer: any) => ({
      ...
      products:
        transfer.items
          ?.map((item: any) => item.product?.product_name)
          .join(", ") || "-",
      units:
        transfer.items?.reduce(
          (sum: number, item: any) => sum + (item.approved_quantity || 0),
          0,
        ) || 0,
  ...
  const filteredData = useMemo(() => {
      ...
      filtered = filtered.filter((item) => {
        switch (activeTab) {
          ...
          case "inventory-movement":
            ...
            return (
              movement.from === selectedBranch || movement.to === selectedBranch
            );
          case "low-stock":
          ...
          case "transfer-history":
            ...
            return (
              transfer.from === selectedBranch || transfer.to === selectedBranch
            );
          default:
            return true;
        }
    ...
    if (selectedStockStatus && activeTab === "stock-summary") {
      filtered = filtered.filter(
        (item) => (item as StockSummaryItem).status === selectedStockStatus,
      );
    }
    ...
    if (selectedMovementType && activeTab === "inventory-movement") {
      filtered = filtered.filter(
        (item) => (item as InventoryMovementItem).type === selectedMovementType,
      );
    }
    ...
    if (selectedDamageStatus && activeTab === "damage-discard") {
      filtered = filtered.filter(
        (item) => (item as DamageDiscardItem).status === selectedDamageStatus,
      );
    }
    ...
    if (selectedTransferStatus && activeTab === "transfer-history") {
      filtered = filtered.filter(
        (item) =>
          (item as TransferHistoryItem).status === selectedTransferStatus,
      );
    }
  ...
  const searchSuggestions = useMemo(() => {
    ...
    currentData.forEach((item) => {
      switch (activeTab) {
        case "stock-summary":
          const stockItem = item as StockSummaryItem;
          if (stockItem.product_name.toLowerCase().includes(query))
            suggestions.add(stockItem.product_name);
          if (stockItem.sku.toLowerCase().includes(query))
            suggestions.add(stockItem.sku);
          break;
        case "inventory-movement":
          const movementItem = item as InventoryMovementItem;
          if (movementItem.product.toLowerCase().includes(query))
            suggestions.add(movementItem.product);
          if (movementItem.type.toLowerCase().includes(query))
            suggestions.add(movementItem.type);
          break;
        case "low-stock":
          const lowStockItem = item as LowStockItem;
          if (lowStockItem.product_name.toLowerCase().includes(query))
            suggestions.add(lowStockItem.product_name);
          if (lowStockItem.sku.toLowerCase().includes(query))
            suggestions.add(lowStockItem.sku);
          break;
        case "transfer-history":
          const transferItem = item as TransferHistoryItem;
          if (transferItem.transfer_number.toLowerCase().includes(query))
            suggestions.add(transferItem.transfer_number);
          if (transferItem.products.toLowerCase().includes(query))
            suggestions.add(transferItem.products);
          break;
      }
    });
  ...
  const handleExportToPDF = () => {
    ...
    try {
      ...
      doc.text(`Total Records: ${filteredData.length}`, 280, yPos, {
        align: "right",
      });
      yPos += 8;
            ...
            <table className="min-w-[800px] md:min-w-full divide-y divide-gray-200">
              <thead>
                <tr className="bg-gray-50">
                  ...
                  <th className="px-2 xs:px-3 sm:px-4 md:px-6 py-2 sm:py-3 text-left text-xs sm:text-sm md:text-md font-medium text-[#37638F] uppercase tracking-wider">
                    Image
                  </th>
                  <th className="px-2 xs:px-3 sm:px-4 md:px-6 py-2 sm:py-3 text-left text-xs sm:text-sm md:text-md font-medium text-[#37638F] uppercase tracking-wider">
                    Product
                  </th>
                  <th className="hidden sm:table-cell px-2 xs:px-3 sm:px-4 md:px-6 py-2 sm:py-3 text-left text-xs sm:text-sm md:text-md font-medium text-[#37638F] uppercase tracking-wider">
                    SKU
                  </th>
                  <th className="hidden lg:table-cell px-2 xs:px-3 sm:px-4 md:px-6 py-2 sm:py-3 text-left text-xs sm:text-sm md:text-md font-medium text-[#37638F] uppercase tracking-wider">
                    Category
                  </th>
                  <th className="px-2 xs:px-3 sm:px-4 md:px-6 py-2 sm:py-3 text-left text-xs sm:text-sm md:text-md font-medium text-[#37638F] uppercase tracking-wider">
                    Branch
                  ...
                  <th className="hidden md:table-cell px-2 xs:px-3 sm:px-4 md:px-6 py-2 sm:py-3 text-left text-xs sm:text-sm md:text-md font-medium text-[#37638F] uppercase tracking-wider">
                    Total
                  </th>
                  <th className="px-2 xs:px-3 sm:px-4 md:px-6 py-2 sm:py-3 text-left text-xs sm:text-sm md:text-md font-medium text-[#37638F] uppercase tracking-wider">
                    Status
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {currentItems.length === 0 ? (
                  ...
                  currentItems.map((item) => {
                    ...
                    return (
                      <tr
                        key={`${product.product_id}-${product.branch_id}`}
                      ...
                      >
                        ...
                        <td className="hidden sm:table-cell px-2 xs:px-3 sm:px-4 md:px-6 py-2 sm:py-4 whitespace-nowrap">
                          <div className="text-xs sm:text-sm md:text-[14px] text-gray-900 font-mono">
                            {product.sku}
                          </div>
                        </td>
                        <td className="hidden lg:table-cell px-2 xs:px-3 sm:px-4 md:px-6 py-2 sm:py-4 whitespace-nowrap">
                          <span className="inline-flex px-2 sm:px-3 py-0.5 sm:py-1 text-xs font-medium">
                            {product.category}
                          </span>
                        </td>
                        <td className="px-2 xs:px-3 sm:px-4 md:px-6 py-2 sm:py-4 whitespace-nowrap">
                        ...
                        <td className="px-2 xs:px-3 sm:px-4 md:px-6 py-2 sm:py-4 whitespace-nowrap">
                          <div className="text-xs sm:text-sm md:text-[14px] font-medium text-green-600">
                      ...
                      <tr key={movement.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-2 xs:px-3 sm:px-4 md:px-6 py-2 sm:py-4 whitespace-nowrap">
                          <div className="text-xs sm:text-sm md:text-[14px] text-gray-900">
                            ...
                            <span className="xs:hidden">
                              {movement.from.substring(0, 3)}→
                            </span>
                            {movement.to}
                          </div>
                        </td>
                        <td className="px-2 xs:px-3 sm:px-4 md:px-6 py-2 sm:py-4 whitespace-nowrap">
                          <div className="text-xs sm:text-sm md:text-[14px] font-semibold text-green-600">
                            {movement.quantity}
                          </div>
                        </td>
                        <td className="hidden lg:table-cell px-2 xs:px-3 sm:px-4 md:px-6 py-2 sm:py-4 whitespace-nowrap">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {currentItems.length === 0 ? (
                  ...
                  currentItems.map((item, index) => {
                    ...
                    return (
                      <tr
                        key={`${lowStockItem.product_id}-${lowStockItem.branch_id}-${index}`}
                        className="hover:bg-gray-50"
                      >
                        <td className="px-2 xs:px-3 sm:px-4 md:px-6 py-2 sm:py-4 whitespace-nowrap">
                          <div className="text-xs sm:text-sm md:text-[14px] font-medium text-gray-900">
                            {lowStockItem.product_name}
                          </div>
                        </td>
                        <td className="hidden sm:table-cell px-2 xs:px-3 sm:px-4 md:px-6 py-2 sm:py-4 whitespace-nowrap">
                          <div className="text-xs sm:text-sm md:text-[14px] text-gray-900 font-mono">
                        ...
                        <td className="hidden md:table-cell px-2 xs:px-3 sm:px-4 md:px-6 py-2 sm:py-4 whitespace-nowrap">
                          <div className="text-xs sm:text-sm md:text-[14px] text-gray-900">
                            {lowStockItem.reorder_point}
                          </div>
                        </td>
                        <td className="hidden lg:table-cell px-2 xs:px-3 sm:px-4 md:px-6 py-2 sm:py-4 whitespace-nowrap">
                          <div className="text-xs sm:text-sm md:text-[14px] font-medium text-red-600">
                            {lowStockItem.quantity_needed}
                          </div>
                        </td>
                        <td className="px-2 xs:px-3 sm:px-4 md:px-6 py-2 sm:py-4 whitespace-nowrap">
                          <span
            ...
            <table className="min-w-[800px] md:min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {currentItems.length === 0 ? (
                  ...
                  currentItems.map((item) => {
                    const damageItem = item as DamageDiscardItem;
                    console.log("damageItem", damageItem);
                    return (
                      <tr key={damageItem.id} className="hover:bg-gray-50">
                        ...
                        <td className="hidden lg:table-cell px-2 xs:px-3 sm:px-4 md:px-6 py-2 sm:py-4 whitespace-nowrap">
                          <div className="text-xs sm:text-sm md:text-[14px] font-semibold text-red-600">
                            -{damageItem.financialImpact}
                          </div>
                        </td>
                        <td className="px-2 xs:px-3 sm:px-4 md:px-6 py-2 sm:py-4 whitespace-nowrap">
                          <div className="text-xs sm:text-sm md:text-[14px] text-gray-900 font-mono">
                            {damageItem.reference}
                          </div>
                        </td>
                      </tr>
                    );
                  ...
                  currentItems.map((item) => {
                    ...
                    return (
                      <tr key={transfer.id} className="hover:bg-gray-50">
                        <td className="px-2 xs:px-3 sm:px-4 md:px-6 py-2 sm:py-4 whitespace-nowrap">
                          <div className="text-xs sm:text-sm md:text-[14px] font-mono text-blue-600">
                        ...
                        <td className="px-2 xs:px-3 sm:px-4 md:px-6 py-2 sm:py-4 whitespace-nowrap">
                          <div className="text-xs sm:text-sm md:text-[14px] text-gray-900">
                            ...
                            <span className="xs:hidden">
                              {transfer.to.substring(0, 3)}
                            </span>
                          </div>
                        </td>
                        <td className="hidden lg:table-cell px-2 xs:px-3 sm:px-4 md:px-6 py-2 sm:py-4">
                          <div
                            className="text-xs sm:text-sm md:text-[14px] text-gray-900 max-w-[200px] truncate"
      ...
      <div className="space-y-4 sm:space-y-6">
        ...
        <div className="px-2 xs:px-3 sm:px-4 md:px-6">
          <div className="flex flex-row items-center">
            ...
            <h1 className="text-base sm:text-xl md:text-2xl font-bold text-gray-900 mx-2">
              Inventory Reports
            </h1>
          </div>
          <p className="text-xs sm:text-sm md:text-base text-[#33333399] mt-1 sm:mt-2 font-semibold">
            Track stock levels, inventory movement, and product performance
            across all branches.
          </p>
        </div>
                ...
                <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
                  ...
                  >
                    <img
                      ...
                      className="w-5 h-5 sm:w-6 sm:h-6"
                    />
                    <span className="hidden xs:inline text-sm font-medium text-black">
                      PDF
                    </span>
                  </button>
                  ...
                  >
                    <img
                      ...
                      className="w-5 h-5 sm:w-6 sm:h-6"
                    />
                    <span className="hidden xs:inline text-sm font-medium text-gray-700">
                      Excel
                    </span>
                  </button>
                </div>
                ...
                <div className="flex items-center flex-wrap justify-center gap-1 sm:gap-2">
                  ...
                  {getPageNumbers().map((pageNumber, index) =>
                    typeof pageNumber === "string" ? (
                      <span
                        key={`ellipsis-${index}`}
                        className="px-2 sm:px-3 py-1 sm:py-1.5 text-xs sm:text-sm text-gray-500"
                      >
                        {pageNumber}
                      </span>
                    ) : (
