...
return (
  ...
  >
    ...
    {/* Popup */}
    <div
      className={`relative bg-white rounded-xl sm:rounded-2xl shadow-2xl w-full max-w-[95%] sm:max-w-md mx-auto p-4 sm:p-6 md:p-8 transform transition-all duration-300 max-h-[90vh] overflow-y-auto ${isClosing ? 'scale-95 opacity-0' : 'scale-100 opacity-100'}`}
    ...
    >
      ...
      {showSuccess && (
        <div className="absolute inset-0 bg-white/95 rounded-xl sm:rounded-2xl flex items-center justify-center z-10 animate-fadeIn">
          <div className="flex flex-col items-center gap-3 sm:gap-4 p-4">
            <img src={add_product_icon} alt="" className="w-12 h-12 sm:w-16 sm:h-16" />
            <p className="text-green-600 font-semibold text-sm sm:text-base">Added successfully!</p>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-0 mb-3 sm:mb-4">
        <div className="w-full sm:w-auto">
          <h2 className="text-lg sm:text-xl font-bold text-gray-900 break-words">{product.name}</h2>
          <p className="text-xs text-gray-500 mt-0.5 break-all">SKU: {currentSku}</p>
        </div>
            ...
            {variants.map((variant) => (
              ...
              >
                <div className="mb-1 sm:mb-0">
                  <span className="font-medium text-gray-900 text-sm sm:text-base">{variant.variant_name}</span>
                  <span className="text-xs text-gray-500 ml-0 sm:ml-2 block sm:inline">SKU: {variant.sku}</span>
                </div>
      ...
      {variants.length === 0 && (
        <div className="mb-4 px-3 py-2 bg-gray-50 rounded-lg">
          <p className="text-xs sm:text-sm text-gray-500">No variants available — single item</p>
        </div>
      )}
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 sm:gap-0">
          <div className="flex items-center justify-center sm:justify-start gap-3">
            <button
              ...
              disabled={quantity <= 1}
              className="w-10 h-10 flex items-center justify-center bg-[#74ABE233] disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer transition-colors font-bold text-lg rounded-lg hover:bg-[#74ABE244]"
            >
              −
            </button>
            <span className="w-14 h-14 flex items-center justify-center bg-[#74ABE2] text-black font-bold text-lg rounded-lg">
              {quantity}
            </span>
            <button
              onClick={() => handleQuantityChange(1)}
              disabled={quantity >= product.stock}
              className="w-10 h-10 flex items-center justify-center bg-[#74ABE233] text-black disabled:opacity-50 cursor-pointer disabled:cursor-not-allowed transition-colors font-bold text-lg rounded-lg hover:bg-[#74ABE244]"
            >
              +
            </button>
          </div>
          <div className="flex items-center justify-between sm:justify-end gap-2 px-2 sm:px-0">
            <p className="text-sm sm:text-base font-semibold text-gray-700">Total:</p>
      ...
      >
        <img src={add_product_icon_2} alt="" className="w-4 h-4 sm:w-5 sm:h-5" />
