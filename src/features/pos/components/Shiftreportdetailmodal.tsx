...
export default function ShiftReportDetailModal({
  ...
  const [activeTab, setActiveTab] = useState<"summary" | "sales" | "movements">(
    "summary",
  );

  // Fetch full register with sales + cash_movements included
  const { data: fullRegisterResponse, isLoading } = useGetPOSByIdQuery(
    reportId!,
    {
  ...
  const knetSales =
    r?.sales
      ?.filter((s: any) => s.payment_method === "K-Net")
      .reduce(
        (sum: number, s: any) => sum + parseFloat(s.total_amount || 0),
        0,
      ) || 0;
  const mobileSales =
    r?.sales
      ?.filter((s: any) => s.payment_method === "Mobile Payment")
      .reduce(
        (sum: number, s: any) => sum + parseFloat(s.total_amount || 0),
        0,
      ) || 0;
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50 p-2 sm:p-4">
      <div className="bg-white rounded-2xl w-full max-w-3xl shadow-2xl overflow-hidden max-h-[95vh] sm:max-h-[90vh] flex flex-col">
          ...
          <div className="min-w-0 flex-1 pr-2 border-b border-gray-300 pb-3">
            ...
            <p className="text-black text-xs sm:text-sm mt-0.5 truncate">
              ...
              {registerData?.opened_at &&
                new Date(registerData.opened_at).toLocaleDateString("en-GB", {
                  day: "2-digit",
                  month: "short",
                  year: "numeric",
                })}
        ...
        <div className="border-b border-gray-300 flex shrink-0 overflow-x-auto scrollbar-hide">
          {(["summary", "sales", "movements"] as const).map((tab) => (
            <button
              ...
              className={`px-4 sm:px-6 py-2.5 sm:py-3 text-xs sm:text-sm font-medium transition-colors border-b-2 whitespace-nowrap ${
                activeTab === tab
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
                    {[
                      ...
                      >
                        ...
                        <p
                          className={`text-base sm:text-xl font-bold truncate ${color}`}
                        >
                          {value}
                        </p>
                      </div>
                  ...
                  <div className="bg-gray-50 rounded-xl p-4 text-sm flex flex-col sm:flex-row justify-between gap-3 sm:gap-0">
                    <div>
                      ...
                      <p className="font-semibold text-gray-800 text-sm sm:text-base">
                        {new Date(r.opened_at).toLocaleTimeString("en-GB", {
                          hour: "2-digit",
                          minute: "2-digit",
                        })}
                      </p>
                    ...
                    <div className="text-right">
                      ...
                      <p className="font-semibold text-gray-800 text-sm sm:text-base">
                        {r.closed_at
                          ? new Date(r.closed_at).toLocaleTimeString("en-GB", {
                              hour: "2-digit",
                              minute: "2-digit",
                            })
                          : "—"}
                  ...
                  <div className="bg-gray-50 rounded-xl p-4 sm:p-5">
                    ...
                    <div className="space-y-2 text-xs sm:text-sm">
                      {[
                        {
                          label: "Opening Balance",
                          value: parseFloat(r.opening_balance || 0),
                        },
                        { label: "Cash Sales", value: cashSales },
                        {
                          label: "Cash In (movements)",
                          value: parseFloat(r.total_cash_in || 0),
                        },
                        {
                          label: "Cash Out (movements)",
                          value: -parseFloat(r.total_cash_out || 0),
                        },
                      ].map(({ label, value }) => (
                        <div key={label} className="flex justify-between">
                          <span className="text-gray-500">{label}</span>
                          <span
                            className={`font-medium ${value < 0 ? "text-red-600" : "text-gray-900"}`}
                          >
                            {value < 0 ? "-" : ""}KD{" "}
                  ...
                  {[
                    {
                      ...
                      amount: cashSales,
                      count:
                        r.sales?.filter((s: any) => s.payment_method === "Cash")
                          .length || 0,
                    },
                    {
                      method: "Card",
                      amount: cardSales,
                      count:
                        r.sales?.filter((s: any) => s.payment_method === "Card")
                          .length || 0,
                    },
                    {
                      method: "K-Net",
                    ...
                    },
                    {
                      ...
                      count:
                        r.sales?.filter(
                          (s: any) => s.payment_method === "Mobile Payment",
                        ).length || 0,
                    },
                  ].some((i) => i.count > 0) && (
                    <div className="bg-gray-50 rounded-xl p-4 sm:p-5">
                        ...
                        {[
                          ...
                          },
                          {
                            ...
                            count:
                              r.sales?.filter(
                                (s: any) =>
                                  s.payment_method === "Mobile Payment",
                              ).length || 0,
                          },
                        ]
                          ...
                          .map(({ method, amount, count }) => (
                            ...
                            >
                              <div className="flex items-center gap-2">
                                <span
                                  className={`px-2 py-0.5 text-xs font-medium rounded-lg ${paymentBadge(method)}`}
                                >
                                  {method}
                                </span>
                    ...
                    <table className="min-w-[500px] sm:min-w-full w-full divide-y divide-gray-200 text-xs sm:text-sm">
                      <thead className="bg-gray-50">
                        <tr>
                          {[
                            ...
                            <th
                              key={h}
                              className="px-3 sm:px-4 py-2.5 sm:py-3 text-left text-xs font-semibold text-gray-500 uppercase whitespace-nowrap"
                            >
                              {h}
                            </th>
                          ))}
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {r.sales.map((sale: any) => (
                          <tr key={sale.id} className="hover:bg-gray-50">
                            <td className="px-3 sm:px-4 py-2.5 sm:py-3 font-medium text-gray-900 whitespace-nowrap">
                              {sale.sale_number}
                            </td>
                            ...
                            <td className="px-3 sm:px-4 py-2.5 sm:py-3 text-gray-700 whitespace-nowrap">
                              {sale.items?.length || 0}
                            </td>
                            <td className="px-3 sm:px-4 py-2.5 sm:py-3 whitespace-nowrap">
                              <span
                                className={`px-2 py-0.5 text-xs font-medium rounded-lg ${paymentBadge(sale.payment_method)}`}
                              >
                                {sale.payment_method}
                              </span>
                            </td>
                      ...
                      <tfoot className="bg-gray-50">
                        <tr>
                          <td
                            colSpan={5}
                            className="px-3 sm:px-4 py-2.5 sm:py-3 font-bold text-gray-700 whitespace-nowrap"
                          >
                            Total
                          </td>
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {r.cash_movements.map((mov: any) => (
                          <tr key={mov.id} className="hover:bg-gray-50">
                            ...
                            <td
                              className={`px-3 sm:px-4 py-2.5 sm:py-3 font-semibold whitespace-nowrap ${mov.type === "Cash In" ? "text-green-600" : "text-red-600"}`}
                            >
                              {mov.type === "Cash In" ? "+" : "-"}KD{" "}
                              {parseFloat(mov.amount || 0).toFixed(3)}
                            </td>
                            <td className="px-3 sm:px-4 py-2.5 sm:py-3 text-gray-700 break-words max-w-[200px] sm:max-w-none">
                              {mov.reason}
                            </td>
                            <td className="px-3 sm:px-4 py-2.5 sm:py-3 text-gray-500 whitespace-nowrap">
                              {mov.user?.email}
        ...
        <div className="px-4 sm:px-6 py-3 sm:py-4 border-t border-gray-300 shrink-0">
          <button
            onClick={onClose}
            className="w-full py-2.5 sm:py-3 border border-gray-300 rounded-xl font-semibold text-gray-700 hover:bg-gray-50 transition-colors text-sm sm:text-base"
          >
            Close
          </button>
        </div>
