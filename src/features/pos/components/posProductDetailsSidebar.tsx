...
console.log('Parsed session:', JSON.parse(localStorage.getItem('pos_session') || '{}'));
  ...
  const employeeDiscount = isEmployeePurchase ? (subtotal - itemDiscountTotal) * 0.2 : 0;
  const total = subtotal - itemDiscountTotal - couponDiscount - employeeDiscount;
 ...
 const handleSetItemDiscount = async (itemId: string, pct: number) => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className={`fixed right-0 top-0 h-full w-full sm:w-[460px] bg-white shadow-2xl transform transition-transform duration-300 ease-in-out z-50 ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          ...
          <div className="p-5 border-b bg-white">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 bg-blue-100 rounded-full flex items-center justify-center">
                  <svg className="w-5 h-5 text-[#1773CF]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="grid grid-cols-3 gap-3 text-xs">
              <div className="bg-gray-50 rounded-lg px-3 py-2">
                ...
                <p className="font-semibold text-gray-800 truncate">{user?.name || 'N/A'}</p>
              </div>
              <div className="bg-gray-50 rounded-lg px-3 py-2">
              ...
              <div className="bg-gray-50 rounded-lg px-3 py-2">
                ...
                <p className="font-semibold text-gray-800">#{registerId || '—'}</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-3">
            {cartItems.length === 0 ? (
              ...
              <>
                {cartItems.map((item) => {
                  const discPct = itemDiscounts[item.id] || 0;
                  const discAmount = (item.price * item.quantity * discPct) / 100;
                      ...
                      <div className="flex gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between">
                            ...
                            <button onClick={() => onRemoveItem(item.id)} className="text-red-400 hover:text-red-600 p-0.5 hover:bg-red-50 rounded transition-colors">
                          ...
                          <div className="flex items-center justify-between mt-2">
                            ...
                            <div className="flex items-center gap-1.5 bg-gray-100 rounded-lg p-0.5">
                              <button onClick={() => onUpdateQuantity(item.id, item.quantity - 1)}
                                className="w-6 h-6 flex items-center justify-center rounded bg-white hover:bg-gray-50 text-gray-700 text-sm font-bold transition-colors">−</button>
                            ...
                            <div className="text-right">
                              <p className="font-bold text-sm text-gray-900">KD {lineTotal.toFixed(3)}</p>
                              {discPct > 0 && (
                                <p className="text-xs text-red-500 line-through">KD {(item.price * item.quantity).toFixed(3)}</p>
                              )}
                            </div>
                          </div>
            ...
            <div className="border-t bg-gray-50 p-5 space-y-4">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <button onClick={() => setGiftReceipt(!giftReceipt)}
                    className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${giftReceipt ? 'bg-[#1773CF]' : 'bg-gray-300'}`}>
                    <span className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white transition-transform ${giftReceipt ? 'translate-x-4.5' : 'translate-x-0.5'}`} />
                  </button>
                  <span className="text-sm text-gray-700">🎁 Gift Receipt</span>
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={() => setIsEmployeePurchase(!isEmployeePurchase)}
                    ...
                    <span className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white transition-transform ${isEmployeePurchase ? 'translate-x-4.5' : 'translate-x-0.5'}`} />
                  </button>
              ...
              {!appliedCoupon ? (
                <div className="flex gap-2">
                  ...
                  <button onClick={handleApplyCoupon} disabled={validatingCoupon || !promoCode.trim()}
                ...
                <div className="flex items-center justify-between bg-green-50 border border-green-200 rounded-lg px-3 py-2">
                  <div>
                    <p className="text-xs text-green-600 font-medium">{appliedCoupon.code} applied</p>
                    <p className="text-sm font-bold text-green-700">-KD {appliedCoupon.discount.toFixed(3)}</p>
                  </div>
                  <button onClick={handleRemoveCoupon} className="text-green-600 hover:text-green-800 text-xs underline">Remove</button>
                </div>
              )}
              ...
              <div className="space-y-1.5 text-sm">
                ...
                <div className="border-t pt-2 flex justify-between font-bold text-base">
                  <span>Total Due</span>
                  <span className="text-[#1773CF] text-lg">KD {total.toFixed(3)}</span>
                </div>
              </div>
