...
const [showBranchDropdown, setShowBranchDropdown] = useState(false);
  ...
  // Check if branch has any open registers
...
const userCanSwitchBranch = canSwitchBranch(user?.role?.role_name);
  ...
  // Redirect ONLY if user HAS an open register and we haven't already redirected
  useEffect(() => {
    // Don't redirect if we've already attempted
    if (redirectAttempted) return;
    
    // Don't redirect while still loading
    if (checkingRegister) return;
    
    // Check if we have a successful response with data (open register exists)
    if (currentRegisterResponse?.success === true && currentRegisterResponse?.data) {
      console.log('User has open register, redirecting to POS:', currentRegisterResponse.data);
      setRedirectAttempted(true);
      navigate(`${basePath}/pos`, { 
        replace: true,
        state: { 
          existingRegister: true,
          register: currentRegisterResponse.data 
        }
      });
    }
    
    // If error is "No open cash register found", that's fine - stay on page
    if (currentRegisterError) {
      console.log('No open register found - staying on terminal page');
    }
  }, [currentRegisterResponse, currentRegisterError, checkingRegister, navigate, basePath, redirectAttempted]);

  useEffect(() => {
  if (branches.length > 0) {
    if (userCanSwitchBranch) {
      // For users who can switch, default to first branch or previously selected
      const defaultBranchId = branches[0]?.id || null;
      setSelectedBranchId(defaultBranchId);
      setSelectedBranch(branches[0]?.branch_name || '');
    } else {
      // For branch-restricted users, use their assigned branch
      if (user?.branch_id) {
  ...
  if (userCanSwitchBranch) {
    ...
    // Update the selected branch name for display
    if (branchId === null) {
  ...
  const handleStartPOS = async (e: React.MouseEvent) => {
    ...
    // Check if branch already has open registers
    ...
    try {
      ...
      console.log('Cash register opened:', response);
  ...
  // Show loading while checking for existing register
  if (checkingRegister) {
    return (
      <DashboardLayout>
          ...
          <div className="">
            <div className="">
              <div className="bg-white px-8 py-4 rounded-lg grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
                ...
                <div className="relative">
  {userCanSwitchBranch ? (
    // Super Admin/Accountant: Interactive dropdown with "All Branches" option
    <>
      <div className="relative">
        <div className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400">
        ...
        <button
          ...
          className="w-full pl-12 pr-10 py-3.5 bg-white text-left border border-gray-200 rounded-sm text-gray-900 font-medium text-base cursor-pointer hover:border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
    ...
    // Branch-restricted users: Display only (not clickable)
    <div className="relative">
      <div className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400">
        <img src={market_icon} alt="" />
      </div>
  ...
  {/* Status messages */}
  {branchesLoading && (
    <p className="text-xs text-blue-600 mt-1 text-center">Loading branches...</p>
  )}
  {branchesError && (
    <p className="text-xs text-red-600 mt-1 text-center">Failed to load branches</p>
  )}
  {!branchesLoading && !branchesError && branches.length === 0 && (
    <p className="text-xs text-yellow-600 mt-1 text-center">No branches found</p>
  )}
  
  {/* Show if branch has open registers */}
