...
export default function ShiftReportsPage() {
  ...
  const totalSales = registers.reduce((sum: number, r: any) => {
    const sales =
      r.sales?.reduce(
        (s: number, sale: any) => s + parseFloat(sale.total_amount || 0),
        0,
      ) || 0;
    return sum + sales;
  }, 0);
  ...
  const filteredRegisters = useMemo(() => {
    ...
    if (startDate) {
      filtered = filtered.filter(
        (r: any) => new Date(r.opened_at) >= new Date(startDate),
      );
    }
    if (endDate) {
      filtered = filtered.filter(
        (r: any) => new Date(r.opened_at) <= new Date(endDate),
      );
    }
    if (searchQuery) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
          ...
          <div className="bg-white rounded-lg p-4">
            ...
            >
              KD{" "}
              {parseFloat(
                dailySummary?.total_cash_difference || totalDifference,
              ).toFixed(3)}
            </p>
          </div>
          <div className="bg-white rounded-lg p-4">
            ...
            <p className="text-3xl font-semibold">
              {dailySummary?.total_transactions ||
                registers.reduce(
                  (sum: number, r: any) => sum + (r.sales?.length || 0),
                  0,
                )}
            </p>
          </div>
                ...
                <thead className="bg-gray-50">
                  <tr>
                    {[
                      ...
                      <th
                        key={h}
                        className="px-4 py-3 text-left text-xs font-medium text-[#37638F] uppercase tracking-wider whitespace-nowrap"
                      >
                        {h}
                      </th>
                    ))}
            ...
            <div className="px-6 py-4 border-t border-gray-300">
              <div className="flex items-center justify-between">
                ...
                <div className="flex space-x-2">
                  ...
                  <span className="px-3 py-1.5 bg-[#1773CF] text-white rounded-lg text-sm font-medium">
                    {currentPage}
                  </span>
                  <button
                    onClick={() =>
