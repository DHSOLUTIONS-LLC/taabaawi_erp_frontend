...
interface CreatePurchaseOrderPageProps {
  isEditMode?: boolean;
  poId?: number;
  updatePO?: (args: { id: number; data: unknown }) => { unwrap: () => Promise<{ data: { id: number } }> };
}
...
export default function CreatePurchaseOrderPage({
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const suppliers: Array<{ id: number; supplier_name: string }> = (suppliersData as any)?.data ?? [];
  const branches: Array<{ id: number; branch_name: string }> = (branchesData as any) ?? [];

  // ── Totals ──────────────────────────────────────────────────
  const subtotal = selectedProducts.reduce((sum, p) => sum + p.price * p.quantity, 0);
  const discount = selectedProducts.reduce(
    (sum, p) => sum + (p.price * p.quantity * (p.discount_percentage || 0)) / 100,
    0
  );
  const tax = selectedProducts.reduce((sum, p) => {
    const afterDisc = p.price * p.quantity * (1 - (p.discount_percentage || 0) / 100);
  ...
  const handleAddProduct = (product: POProduct) => {
    setSelectedProducts((prev) => {
      ...
      if (existing) {
        return prev.map((p) =>
          p.product_id === product.product_id && p.variant_id === product.variant_id
  ...
  const handleUpdatePrice = (id: string, price: number) => {
    if (price < 0) return;
    setSelectedProducts((prev) => prev.map((p) => (p.id === id ? { ...p, price } : p)));
  };

  const handleUpdateDiscount = (id: string, val: number) =>
    setSelectedProducts((prev) =>
      prev.map((p) =>
        p.id === id ? { ...p, discount_percentage: Math.min(100, Math.max(0, val)) } : p
      )
    );
  ...
  const handleUpdateTax = (id: string, val: number) =>
    setSelectedProducts((prev) =>
      prev.map((p) =>
        p.id === id ? { ...p, tax_percentage: Math.min(100, Math.max(0, val)) } : p
      )
    );
  ...
  const validateForm = () => {
    ...
    if (selectedProducts.length === 0) errs.products = 'Add at least one product';
    setErrors(errs);
  ...
  return (
    <DashboardLayout>
  <div className="max-w-7xl mx-auto space-y-4 sm:space-y-6 sm:p-0">

    {/* Header */}
    <div className="flex items-center gap-2 sm:gap-4">
      <button onClick={() => navigate(`${basePath}/purchase/orders`)} className="flex-shrink-0">
      ...
      <div className="lg:col-span-2 space-y-4 sm:space-y-6">
        ...
        <div className="bg-white rounded-xl p-4 sm:p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            ...
            {/* Order Date */}
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-1 sm:mb-2">Order Date</label>
              <input
                type="date"
                value={orderDate}
            ...
            {/* Expected Delivery */}
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-1 sm:mb-2">Expected Delivery</label>
              <input
                type="date"
          ...
          {selectedProducts.length === 0 ? (
            ...
            >
              <div className="w-10 h-10 sm:w-12 sm:h-12 bg-blue-100 rounded-full flex items-center justify-center mx-auto mb-2 sm:mb-3">
                <img src={add_icon} alt="" className="w-5 h-5 sm:w-6 sm:h-6" />
              </div>
              <p className="text-gray-500 font-medium text-sm sm:text-base">No products added yet</p>
              <p className="text-xs sm:text-sm text-gray-400 mt-1">Click here or "Add Product" to get started</p>
            </div>
          ) : (
 ...
 <div className="xl:col-span-4 overflow-x-auto">
                ...
                <table className="w-full">
                  <thead className="bg-gray-50 border-y border-gray-200">
                    <tr>
                      ...
                      <th className="px-3 sm:px-4 py-2 sm:py-3 text-right text-xs font-semibold text-gray-600 uppercase">Unit Price</th>
                      <th className="px-3 sm:px-4 py-2 sm:py-3 text-right text-xs font-semibold text-gray-600 uppercase hidden sm:table-cell">Disc %</th>
                      <th className="px-3 sm:px-4 py-2 sm:py-3 text-right text-xs font-semibold text-gray-600 uppercase hidden md:table-cell">Tax %</th>
                      <th className="px-3 sm:px-4 py-2 sm:py-3 text-right text-xs font-semibold text-gray-600 uppercase">Total</th>
                      <th className="px-3 sm:px-4 py-2 sm:py-3 text-center text-xs font-semibold text-gray-600 uppercase">Del</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100">
                    {selectedProducts.map((product) => {
                      const s = product.price * product.quantity;
                      const d = (s * (product.discount_percentage || 0)) / 100;
                      const t = ((s - d) * (product.tax_percentage || 0)) / 100;
                      ...
                      return (
                        <tr key={product.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-3 sm:px-4 py-3 text-right">
                            <input
                              type="number" min="0" step="0.001" value={product.price}
                          ...
                          <td className="px-3 sm:px-4 py-3 text-right hidden sm:table-cell">
                            <div className="flex items-center justify-end gap-1">
                              <input
                                type="number" min="0" max="100" value={product.discount_percentage || 0}
                          ...
                          <td className="px-3 sm:px-4 py-3 text-right hidden md:table-cell">
                            <div className="flex items-center justify-end gap-1">
                              <input
                                type="number" min="0" max="100" value={product.tax_percentage || 0}
                                onChange={(e) => handleUpdateTax(product.id, parseFloat(e.target.value) || 0)}
                                className="w-14 sm:w-16 px-1.5 sm:px-2 py-1 sm:py-1.5 border border-gray-300 rounded text-right text-xs sm:text-sm focus:ring-1 focus:ring-blue-500"
                              />
                              <span className="text-gray-500 text-xs sm:text-sm">%</span>
                            </div>
          ...
          {errors.products && <p className="text-xs text-red-500 mt-2">{errors.products}</p>}
        </div>
      </div>
      ...
      <div className="space-y-4 sm:space-y-6">
        ...
        <div className="bg-white rounded-xl p-4 sm:p-6">
          ...
          <div className="space-y-2 sm:space-y-3">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-medium text-xs sm:text-sm">{currency} {subtotal.toFixed(3)}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-gray-500 text-xs sm:text-sm">Discount</span>
              <span className="font-medium text-red-600 text-xs sm:text-sm">− {currency} {discount.toFixed(3)}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-gray-500 text-xs sm:text-sm">Tax</span>
              <span className="font-medium text-xs sm:text-sm">{currency} {tax.toFixed(3)}</span>
            </div>
            <div className="flex justify-between text-sm items-center">
              <span className="text-gray-500 text-xs sm:text-sm">Shipping</span>
              <input
                type="number" step="0.001" min="0" value={shippingCost}
            ...
            <div className="flex justify-between pt-2 sm:pt-3 border-t border-gray-200">
              ...
              <span className="text-base sm:text-lg font-bold text-blue-600">{currency} {total.toFixed(3)}</span>
            </div>
        ...
        {/* Terms & Notes */}
        <div className="bg-white rounded-xl p-4 sm:p-6">
          <h2 className="text-base font-semibold text-gray-900 mb-3 sm:mb-4">Terms & Notes</h2>
          <div className="space-y-3 sm:space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-1 sm:mb-2">Terms & Conditions</label>
              <textarea
                value={terms} rows={3}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-1 sm:mb-2">Internal Notes</label>
              <textarea
                value={notes} rows={3}
        ...
        <div className="grid grid-cols-2 gap-3 sm:gap-4">
          ...
          <button
            ...
            disabled={isLoading}
            className="py-2 sm:py-3 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2 text-sm sm:text-base"
          >
            {isLoading && (
            ...
            {isLoading
              ...
              : (isEditMode ? 'Save Changes' : 'Create PO')}
          </button>
        </div>
      </div>
    </div>
