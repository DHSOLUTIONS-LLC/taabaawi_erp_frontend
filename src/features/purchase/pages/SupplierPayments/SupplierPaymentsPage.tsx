...
export default function SupplierPaymentsPage() {
  ...
  const handleDelete = async (id: number) => {
    if (
      !confirm(
        "Are you sure you want to delete this payment? This will update the PO payment status.",
      )
    )
      return;
    try {
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 sm:py-24 text-center px-4">
              ...
              <p className="text-xs sm:text-sm text-gray-400 mt-1 max-w-md">
                Payments appear here once recorded on a Purchase Order
              </p>
            </div>
          ) : (
                ...
                <div className="min-w-[800px] md:min-w-0">
                  <table className="w-full">
                    <thead className="bg-gray-50 border-b border-gray-200">
                      <tr>
                        <th className="px-3 md:px-6 py-2 md:py-4 text-left text-xs font-semibold text-gray-600 uppercase">
                          Payment #
                        </th>
                        <th className="px-3 md:px-6 py-2 md:py-4 text-left text-xs font-semibold text-gray-600 uppercase">
                          PO Number
                        ...
                        <th className="px-3 md:px-6 py-2 md:py-4 text-right text-xs font-semibold text-gray-600 uppercase">
                          Amount
                        </th>
                        <th className="px-3 md:px-6 py-2 md:py-4 text-center text-xs font-semibold text-gray-600 uppercase">
                          Method
                        </th>
                        <th className="px-3 md:px-6 py-2 md:py-4 text-left text-xs font-semibold text-gray-600 uppercase">
                          Reference
                      ...
                      {payments.map((payment: any) => (
                        <tr key={payment.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-3 md:px-6 py-2 md:py-4 text-sm text-gray-900 break-words">
                            {payment.supplier?.supplier_name ?? "N/A"}
                          </td>
