...
export default function SalesDashboardPage() {
  ...
  const { data: invoicesData, refetch: refetchInvoices } = useGetInvoicesQuery({
    ...queryParams,
    invoice_type: queryParams.invoice_type as
      | "b2c"
      | "b2b"
      | "quotation"
      | undefined,
  ...
  useEffect(() => {
    ...
    if (weeklyData?.data?.daily_breakdown) {
      newChartData.weekly.dataPoints = weeklyData.data.daily_breakdown.map(
        (d: any) => ({
      ...
      newChartData.weekly.totalRevenue = parseFloat(
        weeklyData.data.summary?.total_revenue || 0,
      );
      newChartData.weekly.changePercentage =
        weeklyData.data.comparison?.revenue_change_percentage || 0;
    }

    // Process monthly data
    if (monthlyData?.data?.daily_sales) {
      newChartData.monthly.dataPoints = monthlyData.data.daily_sales.map(
        (d: any) => ({
          day: `Day ${d.day}`,
          revenue: parseFloat(d.revenue) || 0,
        }),
      );
      newChartData.monthly.totalRevenue = parseFloat(
        monthlyData.data.summary?.total_revenue || 0,
      );
      newChartData.monthly.changePercentage =
        monthlyData.data.comparison?.revenue_change_percentage || 0;
    }
  ...
  const totalRevenueFromOrders = channelBreakdownArray.reduce(
  ...
  const allTransactions = [
    ...invoices.map((inv: any) => ({
      ...
      type: "Invoice",
      invoiceType:
        inv?.invoice_type === "b2c"
          ? "Sales Invoice"
          : inv?.invoice_type === "b2b"
            ? "B2B Invoice"
            : "Quotation",
  ...
  const filteredTransactions = allTransactions
    .filter((transaction: any) => {
      ...
      if (filters.customerType && filters.customerType !== "Customer Type") {
        if (
          filters.customerType === "B2C" &&
          transaction.customerType !== "B2C"
        )
          return false;
        if (
          filters.customerType === "B2B" &&
          transaction.customerType !== "B2B"
        )
          return false;
      }
      return true;
  ...
  const totalSales =
    invoiceStatsData.by_type?.reduce(
      (sum: number, type: any) => sum + parseFloat(type.total || 0),
      0,
    ) ||
    invoiceStatsData.total_revenue ||
    0;
  ...
  const currentData = {
    ...
    changePercentage:
      processedChartData[selectedTimeframe]?.changePercentage || 0,
      ...
      <div className="space-y-4 md:space-y-6 overflow-x-hidden min-w-0">
          ...
          <div className="bg-white rounded-lg p-4 md:p-5 xl:p-6 min-w-0">
            ...
            <div className="flex items-center mt-2 gap-1">
              ...
              <p className="text-xs md:text-sm font-semibold text-blue-600 truncate">
                {invoiceStatsData.by_type
                  ?.map(
                    (t: any) => `${t.invoice_type?.toUpperCase()}: ${t.count}`,
                  )
                  .join(" · ") || "No invoices"}
              </p>
            </div>
          ...
          >
            {!isExpanded && (
              <div className="min-w-0">
                <h2 className="text-sm md:text-base xl:text-lg font-semibold text-gray-900">
                  Revenue Analysis
                </h2>
              ...
              <div className="xl:col-span-2 bg-white rounded-xl p-4 md:p-5 xl:p-6 min-w-0">
                ...
                <div className="mb-4 md:mb-6">
                  <p className="text-xl md:text-2xl xl:text-3xl font-bold text-gray-900 flex items-center flex-wrap gap-2">
                    KWD{" "}
                    {currentData.totalRevenue.toLocaleString(undefined, {
                      minimumFractionDigits: 3,
                      maximumFractionDigits: 3,
                    })}
                  ...
                  channelBreakdownData.data.channel_breakdown.length > 0 ? (
                    channelBreakdownData.data.channel_breakdown
                      ...
                      .map((channel: any, index: number) => {
                        const totalRevenue =
                          channelBreakdownData.data.summary?.total_revenue || 1;
                        const percentage =
                          (parseFloat(channel.total_revenue) / totalRevenue) *
                          100;
                      ...
                      <div className="absolute inset-0">
                        <svg className="w-full h-full" viewBox="0 0 100 100">
                          <circle
                            ...
                            strokeWidth="6"
                            strokeLinecap="round"
                            strokeDasharray="400.743"
                            strokeDashoffset="155.508"
                      ...
                      <div className="absolute inset-0 p-2">
                        <svg className="w-full h-full" viewBox="0 0 100 100">
                          <circle
                            ...
                            strokeWidth="5"
                            strokeLinecap="round"
                            strokeDasharray="320.911"
                            strokeDashoffset="153.938"
                ...
                <div className="space-y-3">
                  ...
                  channelBreakdownData.data.channel_breakdown.length > 0 ? (
                    channelBreakdownData.data.channel_breakdown.map(
                      (channel: any) => {
                        const totalRev =
                          channelBreakdownData.data.summary?.total_revenue || 1;
                        const percentage = (
                          (parseFloat(channel.total_revenue) / totalRev) *
                          100
                        ).toFixed(1);
                        return (
                          <div
                          ...
                          >
                            <div className="min-w-0">
                              ...
                              <div className="text-xs text-gray-400">
                                {channel.total_orders} orders · KWD{" "}
                                {parseFloat(channel.total_revenue).toFixed(3)}
                              </div>
                            </div>
                    ...
                    <>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="text-xs md:text-sm font-medium text-gray-500">
                          Website: <span>30%</span>
                        </div>
                      </div>
                      <div className="flex items-center justify-between">
                        <div className="text-xs md:text-sm font-medium text-gray-500">
                          Mobile App: <span>25%</span>
                        </div>
                      </div>
                    </>
                  )}
        ...
        <div className="rounded-lg overflow-hidden bg-white min-w-0">
            ...
            <div className="flex flex-wrap gap-3 items-end">
              ...
              <div className="relative w-full sm:w-auto sm:flex-1 sm:min-w-[130px]">
                <select
                  ...
                  onChange={(e) =>
                    handleFilterChange("orderSource", e.target.value)
                  }
                  className="w-full px-4 py-2.5 shadow rounded-lg font-semibold appearance-none bg-white pr-10 text-xs md:text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                >
          ...
          <div className="px-4 md:px-5 xl:px-6 pb-2 flex items-center justify-between">
            <h2 className="text-sm md:text-base xl:text-lg font-bold text-gray-900">
                        ...
                        {showBulkTransfer && (
                          <td className="px-4 md:px-5 py-3 md:py-4 whitespace-nowrap">
                            <input
                              ...
                              onChange={() =>
                                handleProductSelect(transaction.id)
                              }
                              className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500 cursor-pointer"
                            />
                          </td>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-4 md:px-5 py-3 border-t border-gray-100">
            <span className="text-xs md:text-sm text-gray-500">
              ...
              <span className="font-medium">
                {Math.min(recentTransactions.length, 10)}
              </span>{" "}
              of{" "}
