...
export default function CreateOrderPage() {
  ...
  // ─── Redux State ──────────────────────────────────
  const selectedProducts = useAppSelector((state: RootState) => state.sales.orderProducts) || [];
...
console.log('customers',customers)
  ...
  const validateForm = () => {
    ...
    else if (!/\S+@\S+\.\S+/.test(customerEmail)) newErrors.customerEmail = 'Email is invalid';
    ...
    if (!shippingCountry.trim()) newErrors.shippingCountry = 'Shipping country is required';
    if (!channel) newErrors.channel = 'Channel is required';
    if (!paymentMethod) newErrors.paymentMethod = 'Payment method is required';
    if (selectedProducts.length === 0) newErrors.products = 'Add at least one product';
    if (!selectedShippingMethodId) newErrors.shippingMethod = 'Select a shipping method';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (err.data?.errors) {
        ...
        alert(err?.data?.message || 'Failed to create order. Please check all fields and try again.');
      }
    }
  };
  ...
  const PAYMENT_METHODS = ['Cash on Delivery', 'Credit Card', 'Debit Card', 'K-Net', 'Online Payment', 'Bank Transfer'];
  const COUNTRIES = ['Kuwait', 'UAE', 'Saudi Arabia', 'Qatar', 'Bahrain', 'Oman'];
  const KUWAIT_CITIES = ['Kuwait City', 'Hawally', 'Salmiya', 'Farwaniya', 'Jahra', 'Mubarak Al-Kabeer', 'Ahmadi'];
    ...
    <div className="flex items-center justify-between">
      <div className="flex items-center gap-2 sm:gap-4">
        <button onClick={() => navigate(`${basePath}/sales/orders`)}>
          <img src={arrow_back_icon} alt="Back" className="w-6 h-6 sm:w-8 sm:h-8" />
        </button>
        <h1 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900">Create New Order</h1>
      </div>
    </div>
    ...
    <div className="flex flex-col lg:grid lg:grid-cols-3 gap-4 sm:gap-6">
      ...
      <div className="lg:col-span-2 space-y-4 sm:space-y-6">
        ...
        <div className="bg-white rounded-xl p-4 sm:p-6">
          <h2 className="text-base font-semibold text-gray-900 mb-3 sm:mb-4">Order Information</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            ...
            {/* Channel */}
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-1 sm:mb-2">Channel *</label>
              <div className="relative">
                <select
                  value={channel}
            ...
            <div className="sm:col-span-2">
              <label className="block text-sm font-medium text-gray-600 mb-1 sm:mb-2">Branch</label>
              {userCanSwitchBranch ? (
                <div className="relative">
                  <select
                    value={branchId}
        ...
        {/* Customer Info */}
        <div className="bg-white rounded-xl p-4 sm:p-6">
          <h2 className="text-base font-semibold text-gray-900 mb-3 sm:mb-4">Customer Information</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            <div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-1 sm:mb-2">Email *</label>
              <input
                type="email"
                value={customerEmail}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-1 sm:mb-2">Phone *</label>
              <input
                type="text"
                value={customerPhone}
        ...
        {/* Shipping Info */}
        <div className="bg-white rounded-xl p-4 sm:p-6">
          <h2 className="text-base font-semibold text-gray-900 mb-3 sm:mb-4">Shipping Information</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            <div className="sm:col-span-2">
              <label className="block text-sm font-medium text-gray-600 mb-1 sm:mb-2">Shipping Address *</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-1 sm:mb-2">City *</label>
              <div className="relative">
                <select
                  value={shippingCity}
              ...
              {errors.shippingCity && (
                <p className="text-xs text-red-500 mt-1">{errors.shippingCity}</p>
              )}
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-1 sm:mb-2">State/Governorate</label>
              <input
                type="text"
                value={shippingState}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-1 sm:mb-2">Country *</label>
              <div className="relative">
                <select
              ...
              {errors.shippingCountry && (
                <p className="text-xs text-red-500 mt-1">{errors.shippingCountry}</p>
              )}
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-1 sm:mb-2">Postal Code</label>
              <input
                type="text"
              ...
              <div className="min-w-[768px] lg:min-w-full">
                <table className="w-full">
                  <thead className="bg-gray-50 border-y border-gray-200">
                    <tr>
                      <th className="px-3 sm:px-4 py-2 sm:py-3 text-left text-xs font-semibold text-gray-600 uppercase">Product</th>
                      <th className="px-3 sm:px-4 py-2 sm:py-3 text-left text-xs font-semibold text-gray-600 uppercase hidden md:table-cell">SKU</th>
                      <th className="px-3 sm:px-4 py-2 sm:py-3 text-center text-xs font-semibold text-gray-600 uppercase">Qty</th>
                      <th className="px-3 sm:px-4 py-2 sm:py-3 text-right text-xs font-semibold text-gray-600 uppercase">Price</th>
                      <th className="px-3 sm:px-4 py-2 sm:py-3 text-right text-xs font-semibold text-gray-600 uppercase">Total</th>
                      <th className="px-3 sm:px-4 py-2 sm:py-3 text-center text-xs font-semibold text-gray-600 uppercase">Action</th>
                    </tr>
                  </thead>
                    ...
                    {selectedProducts.map((product: any) => {
                      ...
                      return (
                        <tr key={product.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-3 sm:px-4 py-3 text-xs sm:text-sm text-gray-600 hidden md:table-cell">{product.sku}</td>
                          <td className="px-3 sm:px-4 py-3">
                            <div className="flex items-center justify-center gap-1 sm:gap-2">
                              ...
                              <span className="w-6 sm:w-8 text-center font-medium text-sm">{product.quantity}</span>
                              <button
                                onClick={() => handleUpdateQuantity(product.id, product.quantity + 1)}
      ...
      <div className="space-y-4 sm:space-y-6">
        {/* Payment */}
        <div className="bg-white rounded-xl p-4 sm:p-6">
          <h2 className="text-base font-semibold text-gray-900 mb-3 sm:mb-4">Payment</h2>
          <div className="space-y-3 sm:space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-1 sm:mb-2">Payment Method *</label>
              <div className="relative">
                <select
                  value={paymentMethod}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-1 sm:mb-2">Coupon Code</label>
              <input
                type="text"
                value={couponCode}
        ...
        {/* Shipping */}
        <div className="bg-white rounded-xl p-4 sm:p-6">
          <h2 className="text-base font-semibold text-gray-900 mb-3 sm:mb-4">Shipping</h2>
          <div className="space-y-3 sm:space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-1 sm:mb-2">Shipping Method *</label>
              <div className="relative">
                <select
                  value={selectedShippingMethodId}
                  onChange={(e) => {
                    setSelectedShippingMethodId(e.target.value);
                    const method = methods.find((m: ShippingMethod) => m.id.toString() === e.target.value);
                    if (method) {
                      setShippingFee(parseFloat(method.base_cost.toString()));
                    }
                  }}
                  className={`w-full px-3 sm:px-4 py-2 sm:py-3 text-sm border rounded-lg focus:ring-2 focus:ring-blue-500 appearance-none bg-white pr-8 sm:pr-10 ${
                ...
                >
                  ...
                  {methods.map((method: ShippingMethod) => (
                    <option key={method.id} value={method.id}>
                      {method.method_name} — KWD {parseFloat(method.base_cost.toString()).toFixed(3)}
              ...
              {errors.shippingMethod && (
                <p className="text-xs text-red-500 mt-1">{errors.shippingMethod}</p>
              )}
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-1 sm:mb-2">Shipping Fee (KWD)</label>
              <input
                type="number"
                step="0.001"
                value={shippingFee}
        ...
        {/* Order Summary */}
        <div className="bg-white rounded-xl p-4 sm:p-6">
          <h2 className="text-base font-semibold text-gray-900 mb-3 sm:mb-4">Order Summary</h2>
          <div className="space-y-2 sm:space-y-3">
            <div className="flex justify-between text-sm">
              <span className="text-gray-500">Subtotal</span>
              <span className="font-medium text-gray-900">KWD {subtotal.toFixed(3)}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-gray-500">Shipping</span>
              <span className="font-medium text-gray-900">KWD {shippingFee.toFixed(3)}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-gray-500">Tax</span>
              <span className="font-medium text-gray-900">KWD {taxAmount.toFixed(3)}</span>
            </div>
            <div className="flex justify-between pt-2 sm:pt-3 border-t border-gray-200">
              <span className="text-sm font-semibold text-gray-900">Grand Total</span>
              <span className="text-base sm:text-lg font-bold text-blue-600">KWD {grandTotal.toFixed(3)}</span>
            </div>
          </div>
        </div>
        ...
        {/* Customer Notes */}
        <div className="bg-white rounded-xl p-4 sm:p-6">
          <h2 className="text-base font-semibold text-gray-900 mb-3 sm:mb-4">Notes</h2>
          <textarea
            value={customerNotes}
