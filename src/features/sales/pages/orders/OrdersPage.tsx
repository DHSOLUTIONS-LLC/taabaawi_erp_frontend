...
import { useGetOrdersQuery, useGetOrderStatisticsQuery, useDeleteOrderMutation } from '../../../../services/salesApi';
...
console.log('Stats data structure:', stats); 
  ...
  const handleExport = () => {
    try {
      ...
      saveAs(new Blob([buf], { type: 'application/octet-stream' }), `orders_${new Date().toISOString().split('T')[0]}.xlsx`);
...
const pendingCount = orders.filter((order: any) => order.order_status === 'pending').length;
const deliveredCount = orders.filter((order: any) => order.order_status === 'Delivered').length;
  ...
  return (
    <DashboardLayout>
      <div className="min-h-screen space-y-6">
        ...
        {/* Filters */}
        <div className="bg-white px-6 py-4 rounded-xl grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            { label: 'Total Revenue', value: `KD ${Number(totalRevenue).toFixed(3)}`, color: 'text-[#1773CF]' },
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <div className="relative">
                ...
                <input type="text" value={search}
                  ...
                  className="pl-9 pr-4 py-2 border rounded-lg w-56 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              <select value={orderStatus} onChange={e => { setOrderStatus(e.target.value); setCurrentPage(1); }}
                className="px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                <option value="">All Statuses</option>
                {['Pending','Confirmed','Processing','Packed','Shipped','Out for Delivery','Delivered','Cancelled','Returned'].map(s => (
                  <option key={s} value={s}>{s}</option>
                ))}
              </select>
              <select value={paymentStatus} onChange={e => { setPaymentStatus(e.target.value); setCurrentPage(1); }}
                className="px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                <option value="">All Payments</option>
                {['Pending','Paid','Partially Paid','Refunded','Failed'].map(s => (
                  <option key={s} value={s}>{s}</option>
                ))}
              </select>
              <select value={channel} onChange={e => { setChannel(e.target.value); setCurrentPage(1); }}
                className="px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                <option value="">All Channels</option>
                {['Website','Mobile App','POS','Phone','Manual'].map(s => (
                  <option key={s} value={s}>{s}</option>
                ))}
              </select>
            </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  {['Order #', 'Customer', 'Channel', 'Items', 'Total', 'Payment', 'Status', 'Date', 'Actions'].map(h => (
                    <th key={h} className="px-4 py-3 text-left text-xs font-semibold text-[#37638F] uppercase tracking-wider whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
                ...
                {isLoading ? (
                  ...
                  orders.map((order: any) => (
                    <tr key={order.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-4 py-4 text-sm font-semibold text-[#1773CF] cursor-pointer hover:underline"
                        onClick={() => navigate(`${basePath}/sales/orders/${order.id}`)}>
                        {order.order_number}
                      </td>
                      ...
                      <td className="px-4 py-4">
                        <span className="px-2 py-1 text-xs font-medium rounded-lg bg-blue-50 text-blue-700">{order.channel}</span>
                      </td>
                      <td className="px-4 py-4 text-sm text-gray-700">{order.items?.length || 0}</td>
                      <td className="px-4 py-4 text-sm font-semibold text-gray-900">
                        KD {parseFloat(order.total_amount).toFixed(3)}
                      </td>
                      <td className="px-4 py-4">
                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${PAYMENT_STATUS_COLORS[order.payment_status] || 'bg-gray-100 text-gray-600'}`}>
                          {order.payment_status}
                        </span>
                      </td>
                      <td className="px-4 py-4">
                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${ORDER_STATUS_COLORS[order.order_status] || 'bg-gray-100 text-gray-600'}`}>
                          {order.order_status}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-4">
                        <div className="flex gap-2">
                          <button onClick={() => navigate(`${basePath}/sales/orders/${order.id}`)}
