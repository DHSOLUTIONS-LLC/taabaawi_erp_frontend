...
import {
  ...
  useRestoreDeletedRecordMutation,  // uncommented
...
const extractData = (response: any): any[] => {
  if (!response) return [];
  // Case 1: { data: [...] } (non-paginated)
  if (Array.isArray(response.data)) return response.data;
  // Case 2: { data: { data: [...] } } (paginated)
  if (response.data && Array.isArray(response.data.data)) return response.data.data;
  // Case 3: direct array (unlikely but safe)
...
console.log('logs...', logsData)
  ...
  // Login History
  const { data: loginsData } = useGetLoginHistoryQuery(
    { page: currentPage, per_page: 20 },
    { skip: activeTab !== 'logins' }
  );

  // Security Alerts
  const { data: alertsData, refetch: refetchAlerts } = useGetSecurityAlertsQuery(
...
// console.log('alertsData...', alertsData)
  ...
  // Active Sessions
  const { data: sessionsData, refetch: refetchSessions } = useGetActiveSessionsQuery(undefined, {
...
// console.log('active sessions...', sessionsData)
  ...
  // Deleted Records
  const { data: deletedData, refetch: refetchDeleted } = useGetDeletedRecordsQuery(
    { page: currentPage, per_page: 20 },
  ...
  // console.log({ logs, logins, alerts, sessions, deleted });
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <StatCard title="Total Logins" value={stats?.total_logins || 0} icon="🔑" color="bg-blue-500" />
              <StatCard title="Failed Logins" value={stats?.failed_logins || 0} icon="❌" color="bg-red-500" />
              <StatCard title="Suspicious" value={stats?.suspicious_logins || 0} icon="⚠️" color="bg-yellow-500" />
              <StatCard title="Active Sessions" value={stats?.active_sessions || 0} icon="👤" color="bg-green-500" />
              <StatCard title="Unresolved Alerts" value={stats?.unresolved_alerts || 0} icon="🚨" color="bg-purple-500" />
            </div>
            {/* Optional quick actions can be added here */}
          </div>
        )}
                ...
                <tbody className="divide-y">
                  {logins.length === 0 ? (
                    ...
                    <tr key={login.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4">{login.user.name}</td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-sm">{login.ip_address}</td>
                      <td className="px-6 py-4 text-sm">{login.device_type || 'Unknown'}</td>
                      <td className="px-6 py-4 text-sm">
                        {format(new Date(login.login_at), 'MMM dd, HH:mm')}
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <div className="bg-white rounded-lg shadow">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full">
                ...
                <tbody className="divide-y">
                  {deleted.length === 0 ? (
                    <tr><td colSpan={6} className="px-6 py-8 text-center text-gray-500">No deleted records found</td></tr>
                    ...
                    <tr key={record.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 text-sm">
                        {format(new Date(record.deleted_at), 'MMM dd, yyyy')}
                      </td>
                      <td className="px-6 py-4">
                        {record.is_restored ? (
