...
export const hrApi = api.injectEndpoints({
    endpoints: (builder) => ({
        ...
        getAttendanceByDate: builder.query<any, {
            date?: string;
            branch_id?: number;
            page?: number;
            per_page?: number;
        ...
        rejectLeaveRequest: builder.mutation<any, { id: number; rejection_reason?: string }>({
        ...
        // Upload document
        uploadEmployeeDocument: builder.mutation<any, { employeeId: number; data: FormData }>({
            query: ({ data }) => ({
...
getPayrolls: builder.query<any, { 
    ...
    query: (params) => {
        ...
        if (params.payroll_month) queryParams.append('payroll_month', params.payroll_month);
...
generatePayroll: builder.mutation<any, { user_id: number; payroll_month: string }>({
    query: (data) => ({
...
generateBulkPayroll: builder.mutation<any, { payroll_month: string; branch_id?: number }>({
    query: (data) => ({
...
markPayrollAsPaid: builder.mutation<any, { id: number; payment_date: string }>({
