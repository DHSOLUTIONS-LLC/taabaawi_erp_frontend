...
export interface SavedReport {
  ...
  report_name: string;
  report_code: string;
  report_type: 'Sales Report' | 'Inventory Report' | 'Financial Report' | 'HR Report' | 'Customer Report' | 'Custom Report';
  description?: string;
  data_source: string;
  filters?: any;
  columns?: any;
  ...
  calculations?: any;
  custom_query?: string;
  chart_type: 'None' | 'Bar Chart' | 'Line Chart' | 'Pie Chart' | 'Area Chart' | 'Table';
...
export interface KpiMetric {
  ...
  kpi_name: string;
  kpi_code: string;
  kpi_category: 'Sales' | 'Financial' | 'Inventory' | 'HR' | 'Customer' | 'Operations';
  description?: string;
...
export const reportsApi = api.injectEndpoints({
  ...
  endpoints: (builder) => ({
    ...
    getReports: builder.query<{ data: SavedReport[]; meta?: any }, {
      report_type?: string;
      visibility?: string;
      is_active?: boolean;
      search?: string;
    ...
    updateReport: builder.mutation<{ data: SavedReport }, { id: number; data: Partial<SavedReport> }>({
    ...
    getExecutionHistory: builder.query<{ data: ReportExecution[]; meta?: any }, {
      id: number;
      per_page?: number;
    }>({
      query: ({ id, ...params }) => ({ url: `/reports/${id}/execution-history`, params }),
      transformResponse: normalizePaginator,
    ...
    getSalesSummary: builder.query<any, { start_date?: string; end_date?: string }>({
      query: (params) => ({ url: '/pre-built-reports/sales-summary', params }),
      transformResponse: (response: any) => response.data,
    }),

    getTopSellingProducts: builder.query<any, {
      start_date?: string;
      end_date?: string;
      limit?: number;
    }>({
      query: (params) => ({ url: '/pre-built-reports/top-selling-products', params }),
      transformResponse: (response: any) => response.data,
    }),

    getInventoryStatus: builder.query<any, { low_stock_threshold?: number }>({
      query: (params) => ({ url: '/pre-built-reports/inventory-status', params }),
      transformResponse: (response: any) => response.data,
    }),

    getCustomerAnalysis: builder.query<any, { start_date?: string; end_date?: string }>({
      query: (params) => ({ url: '/pre-built-reports/customer-analysis', params }),
      transformResponse: (response: any) => response.data,
    }),

    getEmployeePerformance: builder.query<any, { start_date?: string; end_date?: string }>({
      query: (params) => ({ url: '/pre-built-reports/employee-performance', params }),
      transformResponse: (response: any) => response.data,
    }),

    getFinancialSummary: builder.query<any, { start_date?: string; end_date?: string }>({
      query: (params) => ({ url: '/pre-built-reports/financial-summary', params }),
      transformResponse: (response: any) => response.data,
    }),

    // ── Dashboards ──────────────────────────────────────────────────────────

    getDashboards: builder.query<{ data: Dashboard[]; meta?: any }, {
      visibility?: string;
      is_active?: boolean;
      per_page?: number;
    }>({
    ...
    getDashboard: builder.query<{ dashboard: Dashboard; widget_data: any }, number>({
    ...
    updateDashboard: builder.mutation<{ data: Dashboard }, { id: number; data: Partial<Dashboard> }>({
      query: ({ id, data }) => ({ url: `/dashboards/${id}`, method: 'PUT', body: data }),
      invalidatesTags: (_r, _e, { id }) => ['Dashboards', { type: 'Dashboards', id }],
    }),
    ...
    addWidget: builder.mutation<{ data: Dashboard }, {
      id: number;
      widget_type: 'kpi' | 'chart' | 'table' | 'text';
      widget_title: string;
      widget_config?: any;
    }>({
      query: ({ id, ...body }) => ({ url: `/dashboards/${id}/widgets`, method: 'POST', body }),
      invalidatesTags: (_r, _e, { id }) => [{ type: 'Dashboards', id }],
    }),

    removeWidget: builder.mutation<{ data: Dashboard }, { id: number; widget_id: string }>({
      query: ({ id, widget_id }) => ({ url: `/dashboards/${id}/widgets`, method: 'DELETE', body: { widget_id } }),
      invalidatesTags: (_r, _e, { id }) => [{ type: 'Dashboards', id }],
    }),

    updateWidget: builder.mutation<{ data: Dashboard }, {
      id: number;
      widget_id: string;
      widget_config: any;
    }>({
      query: ({ id, ...body }) => ({ url: `/dashboards/${id}/widgets`, method: 'PUT', body }),
      invalidatesTags: (_r, _e, { id }) => [{ type: 'Dashboards', id }],
    }),

    // ── KPI Metrics ─────────────────────────────────────────────────────────

    getKpiMetrics: builder.query<{ data: KpiMetric[]; meta?: any }, {
      kpi_category?: string;
      is_active?: boolean;
      per_page?: number;
    }>({
    ...
    updateKpiMetric: builder.mutation<{ data: KpiMetric }, { id: number; data: Partial<KpiMetric> }>({
      query: ({ id, data }) => ({ url: `/kpi-metrics/${id}`, method: 'PUT', body: data }),
      invalidatesTags: (_r, _e, { id }) => ['KpiMetrics', { type: 'KpiMetrics', id }],
    }),
    ...
    getExportHistory: builder.query<{ data: ReportExecution[]; meta?: any }, {
      reportId: number;
      per_page?: number;
    }>({
      query: ({ reportId, ...params }) => ({ url: `/report-export/${reportId}/history`, params }),
      transformResponse: normalizePaginator,
    }),
