...
export const securityApi = api.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getUserActivity: builder.query<
      { data: ActivityLog[] },
      { userId: number; params?: ActivityLogFilters }
    >({
      query: ({ userId, params }) => ({
    ...
    getUserLoginHistory: builder.query<
      { data: LoginHistory[] },
      { userId: number; params?: LoginHistoryFilters }
    >({
      query: ({ userId, params }) => ({
        url: `/security/login-history/user/${userId}`,
        params,
      }),
      providesTags: ["LoginHistory"],
    }),
    ...
    markAlertFalsePositive: builder.mutation<
      { data: SecurityAlert },
      { id: number; resolution_notes?: string }
    >({
      query: ({ id, resolution_notes }) => ({
    ...
    getModelChangeHistory: builder.query<
      { data: ChangeHistory[] },
      { model_type: string; model_id?: number }
    >({
    ...
    getDeletedRecordStatistics: builder.query<
      { data: DeletedRecordStatistics },
      { start_date?: string; end_date?: string }
    >({
